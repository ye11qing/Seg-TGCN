...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.lucky = nn.Embedding(configs.enc_in, configs.d_model // 2)###嵌入层

        self.seq_len = configs.seq_len
        self.pred_len = configs.pred_len
        self.enc_in = configs.enc_in        #'--enc_in', type=int, default=7, help='channel or dimension'
    ...
    def forward(self, x, x_mark, y_true, y_mark):
        ...
        B, L, C = x.shape           # batch_size, seq_len, channels
        N = self.seq_len // self.patch_len
        M = self.pred_len // self.patch_len
        W = self.patch_len
        d = self.d_model            # '--d_model', type=int, default=256, help='dimension of model'
        # self.seq_len = L,

        xw = x.permute(0, 2, 1).reshape(B * C, N, -1)  # B, L, C -> B, C, L -> B * C, N, W
        xd = self.linear_patch(xw)  # B * C, N, W -> B * C, N, d, 通过了一个线性层
        enc_in = self.relu(xd)# B * C, N, d
